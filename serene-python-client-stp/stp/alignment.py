...
from serene.elements.elements import DataNode
...
def construct_data_node_map(integration_graph):
    ...
    for n_id, n_data in integration_graph.nodes(data=True):
        ...
        if n_data["attr_dict"]["type"] == "DataNode":
            prefix = n_data["attr_dict"]["prefix"]
            data_node_map[prefix + n_data["attr_dict"]["lab"]].append(n_id)
        elif n_data["attr_dict"]["type"] == "ClassNode":
            prefix = n_data["attr_dict"]["prefix"]
            data_node_map[prefix + n_data["attr_dict"]["lab"]].append(n_id)
        elif n_data["attr_dict"]["type"] == "Attribute":
...
def get_weight(source, target, label, integration_graph):
    ...
    if links:
        for l in links.values():
            if l["attr_dict"]["label"] == label:
                return l["attr_dict"]["weight"]
        return 0
...
def convert_ssd(ssd, integration_graph, file_name=None, clean_ssd=True):
    ...
    for source, target, l_data in ssd.semantic_model._graph.edges(data=True):
        ...
        if isinstance(data, ObjectLink):
            ...
            class_node = g.nodes[ssd_node_map[source]]
            data_node = ssd.semantic_model._graph.nodes[target]["data"]
            link_label = data.label
            full_data_label = class_node["attr_dict"]["prefix"] + class_node["attr_dict"]["lab"] + "---" + link_label
            ...
            node_data = {
                "type": "DataNode",
                "label": class_node["attr_dict"]["label"] + "---" + link_label,
                "lab": class_node["attr_dict"]["lab"] + "---" + link_label,
                "prefix": class_node["attr_dict"]["prefix"]
            }
            g.add_node(node_id, attr_dict=node_data)
    ...
    for source, target, l_data in graph.edges(data=True):
        ...
        if target not in pgv_nodes:
            ...
            g.add_node(target, label=str(target)+"\n"+n["label"],
                       **styles[graph.nodes[target]["type"]])
...
def semantic_label_df(cur_ssd, col_name="pred_label"):
    ...
    # for k, v in cur_ssd.mappings.items():
    #     print("klabel: ", k.label)
    #     print("Ktype: ", type(k))
    #     print("k_: ", k)
    # ff = [(v.id, k._class_node.label + "---" + k.label) if type(k) == DataNode else (v.id, k.label + "---" + k.label) for (k, v) in cur_ssd.mappings.items()]
    ff = []
    for k, v in cur_ssd.mappings.items():
        if type(k) == DataNode:
            ff.append((v.id, k._class_node.label + "---" + k.label))
        elif type(k) == ClassNode:
            ff.append((v.id, k.label + "---" + k.label))
        else:
            ff.append((v.id, k.name + "---" + k.name))
...
def compare_semantic_label(one_df, two_df):
    ...
    # print("merged: ", merged)
    y_true = merged["user_label"].astype(str).to_numpy() 
    y_pred = merged["label"].astype(str).to_numpy() 
...
def do_chuffed(server, ch_octopus, ch_dataset, ch_column_names, ch_ssd, orig_ssd,
    ...
    cor_match_score = sum([ld["attr_dict"]["weight"] for n1, n2, ld in nx_ssd.edges(data=True) if ld["attr_dict"]["type"] == "MatchLink"])
    cor_cost = sum([ld["attr_dict"]["weight"] for n1, n2, ld in nx_ssd.edges(data=True) if ld["attr_dict"]["type"] != "MatchLink"])
