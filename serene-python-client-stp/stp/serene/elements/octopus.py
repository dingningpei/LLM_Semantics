...
class Octopus(object):
    ...
    def train(self):
        ...
        def is_finished():
            ...
            print("state().status: ", self._get_state().status)
    ...
    def convert_karma_graph(data):
        ...
        for link in links:
            ...
            if link["type"] in ["DataPropertyLink", "ClassInstanceLink"]:
                # change data nodes
                # print("source, target, link_data: ", g.nodes[node_map[source]], g.nodes[node_map[target]], link_data)
                g.nodes[node_map[target]]["attr_dict"]["label"] = g.nodes[node_map[source]]["attr_dict"]["label"] + "---" + link_data["label"]
                g.nodes[node_map[target]]["attr_dict"]["lab"] = g.nodes[node_map[source]]["attr_dict"]["lab"] + "---" + link_data["label"]
                g.nodes[node_map[target]]["attr_dict"]["prefix"] = g.nodes[node_map[source]]["attr_dict"]["prefix"]
