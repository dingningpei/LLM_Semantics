...
class SSD(object):
    ...
    def map(self, column, data_node):
        ...
        # print("column, data_node   ", column, data_node)
...
class SSDGraph(object):
    ...
    def add_edge(self,
        ...
        if i_s not in self._graph.nodes:
            ...
            selected = list(self._graph.nodes())[0]
            msg = "Link failed. Could not find source node {} in semantic model and just add to {}".format(i_s, selected)
            print("msg: ", msg)
            i_s = selected
            # raise Exception(msg)
    ...
    def all_neighbors(self, n):
        ...
        return list(self._graph.successors(n)) + list(self._graph.predecessors(n))
    ...
    def edge_data(self, src, dst, index=0):
        ...
        return self._graph.edges[src][dst][index][self.DATA_KEY]

    def all_edge_data(self, src, dst):
        """Helper function to return all the data stored on the edge"""
        # print("src: ", src)
        # print("dst: ", dst)
        # print("edges: ", type(list(self._graph.edges)))
        # print(src in list(self._graph.edges))
        if src in list(self._graph.edges) and dst in self._graph.edges[src]:

            z = self._graph.edges[src][dst]
    ...
    def _edge_list(self):
        ...
        edge_dicts = self._graph.edges(keys = True,  data=True)
        # arr = []
        # for x, y, z in edge_dicts:
        #     for k, v in z.items():
        #         arr.append((x, y, k, v[self.DATA_KEY]))
        # for x, y, k, z in edge_dicts:
        #     arr.append((x, y, k, z[self.DATA_KEY]))

        # return arr
        # print(self._graph.edges(keys = True, data=True))
        # print([(x[0], x[1], x[2], x[3][self.DATA_KEY])  for  x in edge_dicts])
        return [(x[0], x[1], x[2], x[3][self.DATA_KEY])  for  x in edge_dicts]
    ...
    def mappings(self):
        ...
        def data(edge):
            return self._graph.nodes[edge][self.DATA_KEY]
...
class SSDReader(object):
    ...
    def _build_graph_nodes(self, nodes):
        ...
        for obj in nodes:
            if "attr_dict" in obj.keys():
                index = obj["id"]
                prefix = obj["attr_dict"]["prefix"] if 'prefix' in obj else self._ns
                label = obj["attr_dict"]["label"]
                if obj["attr_dict"]['type'] == "ClassNode":
