...
if "SERENE_PYTHON" not in os.environ:
    os.environ["SERENE_PYTHON"] = "/Users/dingningpei/Desktop/Prompt_Semantic/serene-python-client-stp"
...
with open(result_csv, "w+") as f:
    ...
    # for cur_id in sample_range: #sample_range[:len(sample_range)//2]:
    ...
    #         # train_sample = sample_range[len(sample_range)//2: ]
    #         print("     train sample size: ", len(train_sample))
    
    for cur_id in sample_range[:14]: #sample_range[:len(sample_range)//2]:
        print("Currently selected ssd: ", cur_id)
        test_sample = [cur_id]
        ...
        for num in [14]:
            ...
            chuffed_paths = [
                (os.path.join(chuffed_lookup_path, "chuffed_20170525"),
                 True, None, False, 1.0)
