...
//Clause_new(v);
#define GiveFailureExplanation(v)  Clause *__expl = __clause_new(this); \ 
                                   __expl->temp_expl = 1; \
                                   sat.rtrail.last().push(__expl); \
                                   sat.confl = __expl;

#define ReasonNew(v)               __reason_new(this);//Reason_new(v);       

Clause* __reason_new(BoundedPathPropagator* p) {
    vector<Lit> x = p->fullExpl(false);
    return Reason_new(x);
}
Clause* __clause_new(BoundedPathPropagator* p) {
    vector<Lit> x = p->fullExpl(true);
    return Clause_new(x);
}
