...
public:
    ...
    bool propagate() {
        //cout<<"Ppgate "<<engine.decisionLevel()<<endl;
...
void steinerTree(vec<BoolView>& _vs, vec<BoolView>& _es, 
                 vec< vec<edge_id> >& _adj, vec< vec<int> >& _en,
                 IntVar* _w, vec<int> _ws) {
