...
class CircuitKF : public Propagator {
public:
    ...
    IntView<> * const x; // successor variables
    ...
    bool cyclePrevent() {
      ...
      for (int unfixedVar=0; unfixedVar < size; unfixedVar++) {
        ...
        for (IntView<>::iterator it = x[unfixedVar].begin(); it != x[unfixedVar].end(); ++it) 
        {
   ...
   // fprintf(stderr, "explaining");
    ...
    bool exploreSubtree(int thisNode, int startPrevSubtree, int endPrevSubtree, int *backfrom, int *backto, int *numback)
    {
        ...
        int child;
          for (IntView<>::iterator i = x[thisNode].begin(); i != x[thisNode].end(); ++i) 
        {
            child = *i;
    ...
    bool circuitSCC(int rootEnd) {
       ...
       // fprintf(stderr, "rootEnd %d\n", rootEnd);
        ...
        for (IntView<>::iterator i = x[rootEnd].begin(); i != x[rootEnd].end(); ++i) 
        {
              int child = *i;
	...
	for(int i = 0; i < size; i++) {
	    //if (x[i].isFixed())
                fprintf(stderr, "Domain of %d is {", i);
            ...
            {//fprintf(stderr,"nextvar %d ", nextVar);
                ...
                switch(so.checkfailure)
                {
                    ...
                    case 7: //cycle with highest ave activity
                        act = sat.getLevel(var(x[nextVar].getValLit()));
                        thisScore += act;
                        break;
