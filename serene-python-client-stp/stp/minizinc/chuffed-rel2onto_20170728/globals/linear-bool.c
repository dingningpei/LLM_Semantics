...
public:
	...
	void wakeup(int i, int c) {
            if (i < x.size()) {
                ones++;
                assert(x[i].isFixed()); //Added by me, because of old version with caching
                fixed ^= bit32[i];
                if (x[i].getVal() == 0) zeros++;
            }
		pushInQueue();
	}
	...
	void getKey() {
            ...
            int a0 = 0 - (y.isFixed() ? y.getVal() : 0) - sum;
            core_key.push(a0);
            return;
            //assert(false);
		if (redundant) return;
		if (x.size()-zeros <= y.getMin()) {
			redundant = 1; return;
		}
		if (!fixed) return;
		core_key.push(prop_id);
		core_key.push(low((int64_t)fixed));
		core_key.push(high((int64_t)fixed));
		core_key.push(ones);
	}
