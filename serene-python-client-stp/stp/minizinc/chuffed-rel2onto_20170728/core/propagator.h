...
class Propagator : public Keyable {
public:
	int const prop_id;
	int priority;
	...
	Propagator() : prop_id(engine.propagators.size()), priority(0),
            satisfied(false), in_queue(false), simplified(false) {
	...
	virtual int checkSatisfied() { return 0; }
        ...
        // The engine lets the propagator know that tehre is a new decision
	virtual void notifyNewDecLevel() { }

	// Print meaning of literal
	virtual void printLit(int v, bool sign) { NEVER; }

        //Check if the constraint is satisfied once the final assignment is done
        virtual bool checkFinalSatisfied() { return true; }
