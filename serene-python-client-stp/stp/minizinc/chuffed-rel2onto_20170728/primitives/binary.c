...
public:
	...
	BinGE(IntView<U> _x, IntView<V> _y, BoolView _r = bv_true) :
		x(_x), y(_y), r(_r) {
            if (r.isFixed() || x.isFixed() || y.isFixed()) engine.keyables.pop();
	...
	int checkSatisfied() {
        ...
        void getKey() {
            ...
            if (!r.isFixed()) {
                ...
                int a0 = 0 - (y.isFixed() ? y.getVal() : 0) + (x.isFixed() ? x.getVal() : 0);
            ...
            // Binary constraint has no key
            if (!R) return;
            // Only cases where constraint is not redundant
            if (!r.isFixed() && x.isFixed() && !y.isFixed()) {
                core_key.push(prop_id);
                core_key.push(x.getVal() + (0<<30));
            }
            if (!r.isFixed() && !x.isFixed() && y.isFixed()) {
                core_key.push(prop_id);
                core_key.push(y.getVal() + (1<<30));
            }
            if (r.isTrue() && !x.isFixed() && !y.isFixed()) {
                core_key.push(prop_id);
                core_key.push(r.getVal() + (2<<30));
            }
	}
...
void int_rel(IntVar* x, IntRelType t, IntVar* y, int c) {
	...
	switch (t) {
		...
		case IRT_NE:
			break;
		case IRT_LE:
			if(so.mip) mip->addConstraint(coeffs, vars, -1e100, c);
			break;
		case IRT_LT:
			if(so.mip) mip->addConstraint(coeffs, vars, -1e100, c-1);
			break;
		case IRT_GE:
			if(so.mip) mip->addConstraint(coeffs, vars, c, 1e100);
			break;
		case IRT_GT:
			if(so.mip) mip->addConstraint(coeffs, vars, c+1, 1e100);
			break;
		default: NEVER;
	}
