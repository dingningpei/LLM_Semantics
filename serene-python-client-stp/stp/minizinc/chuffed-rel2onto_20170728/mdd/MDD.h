...
public:
  ...
  MDD(void)
    : table( NULL ), val( -1 )
  {  }  
    static MDD read_mdd_file(std::string filename);
    int count_solutions();
  MDDTable* table;
  _MDD val;
...
//   _MDD tuple(std::vector<int>&);
   ...
   MDD varle(int var, int val) { return MDD(this, mdd_varlt(var, val+1)); }
   MDD varge(int var, int val) { return MDD(this, mdd_vargt(var, val-1)); }
   ...
   _MDD mdd_varrange(int var, int lb, int ub);
