...
void mdd_decomp_dc(vec<IntVar*> xs, MDDTable& t, _MDD root, const MDDOpts& mopts)
{
  vec< intpair > ranges;
  vec<int> domain_sizes;
  ...
  if ( mopts.decomp == MDDOpts::D_MINIMAL||
       mopts.decomp == MDDOpts::D_MINISAT) {

    //printf("cls11: (parent , x_i) -> child \n");
    for(int ei = 0; ei < edges.size(); ei++)
    {
      vec<Lit> cl;
      inc_edge& e(edges[ei]);
      Lit vlit = xs[vals[e.val].var]->getLit(vals[e.val].val, 1);
      cl.push(~nodevars[e.begin]);
      cl.push(~vlit);
      cl.push(nodevars[e.end]);
      sat.addClause(cl);
    }

    if ( mopts.decomp == MDDOpts::D_MINISAT) {
        //printf("cls13: (child, x_i) -> parent \n");
        for(int ei = 0; ei < edges.size(); ei++)
        {
          vec<Lit> cl;
          inc_edge& e(edges[ei]);
    ...
    for(int ni = 0; ni < nodes.size(); ni++) {
        ...
        if(node.num_out > 0) {
         ...
         //   if ( mopts.decomp == MDDOpts::D_MINISAT) {
         //       //printf("cls15: n <- (c_1, c_2 ... ) \n");
         //     if(domain_sizes[ni] == node.num_out) {// all siblings exist
         //       vec<Lit> cl;
         //       cl.push(nodevars[ni]);
         //       for(int ci = node.out_start, cidx = 0; cidx < node.num_out; ci++, cidx++)
         //       {
  ...
  if ( mopts.decomp == MDDOpts::D_LEVEL ||
       mopts.decomp == MDDOpts::D_COMPLETE ) {
      //printf("cls09: EO_x(n | var(n)=x)\n");
      ...
      for( int i = 0; i < var_nodes.size(); i++ ) {
          if (var_nodes[i].size() > 1) {
              // add EO per level (quadratic  encoding)
              vec<Lit> cl;
              for( int j = 0; j < var_nodes[i].size()-1; j++ ) {
                  cl.push(nodevars[var_nodes[i][j]]);
                  for( int k = j+1; k < var_nodes[i].size(); k++ ) {
                      sat.addClause(~nodevars[var_nodes[i][j]],~nodevars[var_nodes[i][k]]);
                  }
              }
              cl.push(nodevars[var_nodes[i].last()]);
              sat.addClause(cl);
          }
          //printf("%d: %d\n", i,var_nodes[i].size());

          //if (var_nodes[i].size() > 1) {
          //  // add EO per level (ladder encoding)
      	  //  vec<Lit> aux_level;
  	 	  //  for(int ni = 0; ni < var_nodes[i].size(); ni++)
  	 	  //  aux_level.push(Lit(sat.newVar(), 1));
          //  vec<Lit> unit;
          //  unit.push(aux_level.last());
          //  sat.addClause(unit);

          //  for( int j = 0; j < var_nodes[i].size()-1; j++ ) {
          //      sat.addClause(~nodevars[var_nodes[i][j]], aux_level[j] );
          //      if (j > 0 ) {
          //         sat.addClause(~nodevars[var_nodes[i][j]], aux_level[j-1] );
          //         sat.addClause(~aux_level[j-1], aux_level[j]);
          //      }
          //  }
          //}
      }
  ...
  if ( mopts.decomp == MDDOpts::D_TSEITIN ||
       mopts.decomp == MDDOpts::D_BASIC ||
       mopts.decomp == MDDOpts::D_SDNNF ||
       mopts.decomp == MDDOpts::D_LEVEL ||
       mopts.decomp == MDDOpts::D_COMPLETE ) {
    ...
    for(int ei = 0; ei < edges.size(); ei++)
    {
      ...
      if ( mopts.decomp == MDDOpts::D_TSEITIN ) {
          vec<Lit> cl;
          //printf("cls16: (child, x_i) -> e\n");
          cl.push(edgevars[ei]);
          cl.push(~vlit) ;
          cl.push(~nodevars[e.end]);
      }

      if ( mopts.decomp == MDDOpts::D_BASIC ||
           mopts.decomp == MDDOpts::D_SDNNF ||
           mopts.decomp == MDDOpts::D_LEVEL ||
           mopts.decomp == MDDOpts::D_COMPLETE ) {
          vec<Lit> cl;
          //printf("cls03: (parent,x_i) -> e\n");
          cl.push(edgevars[ei]);
          cl.push(~vlit) ;
          cl.push(~nodevars[e.end]);
      }
    ...
    if ( mopts.decomp == MDDOpts::D_SDNNF ||
         mopts.decomp == MDDOpts::D_COMPLETE ) {
        //printf("cls10: x_i -> ( e | e=(_,x_i,_))\n");
...
void mdd_decomp_dc(vec<IntView<4> > xs, MDDTable& t, _MDD root, const MDDOpts& mopts)
{
  vec< intpair > ranges;
  vec<int> domain_sizes;
  for( int ii = 0; ii < xs.size(); ii++ )
