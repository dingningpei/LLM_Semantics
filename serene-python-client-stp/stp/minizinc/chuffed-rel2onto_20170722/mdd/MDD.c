...
_MDD MDDTable::mdd_varrange(int var, int lb, int ub)
{
  if(ub < lb)
    return MDDFALSE;

  unsigned int start = stack.size();
  stack.push_back(mkedge(lb, MDDTRUE));
...
_MDD MDDTable::bound(_MDD root, vec< intpair >& range)
{
  // return root;
  _MDD bmdd = MDDTRUE;
  for(int v = range.size()-1; v >= 0; v--)
    bmdd = mdd_and(mdd_varrange(v, range[v].first, range[v].second), bmdd);
  return mdd_and(bmdd, root);
}
...
{
   if(r == MDDFALSE) { std::cout << "FFF" << std::endl; return; }
   if(r == MDDTRUE) { std::cout << "TTT" << std::endl; return; }
...
#include <set>
MDD MDD::read_mdd_file(std::string filename) {
    std::unordered_map<int,int> shifts;
                ...
                if (node == -1) {
                    ...
                    if (pch[0] == '.') {
                        ...
                        if (val == -1) {
                            ...
                            pch = strtok (NULL, ")");
                            dest = atoi(pch);
                            edges[node].push_back(std::make_pair(val,dest));
                            pch = strtok (NULL, "(");
                            if (pch != NULL)
                                pch = strtok (NULL, ",");
                            val = -1;
    ...
    MDDTable* tab  = new MDDTable(vars.size());
    _MDD node = MDDTRUE;
    ...
    for (int i = node_ids.size()-1; i >= 0; i--) {
        ...
        _MDD old = node_ids[i];
        std::vector<std::pair<int,int> >& e = edges[old];
...
int MDD::count_solutions() {
    int res = 0;

    int n = this->val;
    
    if (n == MDDTRUE)  return 1;
    if (n == MDDFALSE) return 0;
    
    const std::vector<MDDNode>& nodes = this->table->getNodes();
    for(int d = 0; d < nodes[n]->sz-1; d++)
    {
        MDD next;
        next.table = this->table;
        next.val = nodes[n]->edges[d].dest;
        assert(nodes[n]->edges[d+1].val >  nodes[n]->edges[d].val);
        res += next.count_solutions()*(nodes[n]->edges[d+1].val - nodes[n]->edges[d].val);
    }
