...
public:
	...
	void getKey() {
		if (redundant) return;
		if (z.isFixed() && z.getVal() == 0 && ((x.isFixed() && x.getVal() == 0) || (y.isFixed() && y.getVal() == 0))) {
                    redundant = 1; return;
		}
		if (x.isFixed() && y.isFixed() && z.isFixed() && x.getVal() * y.getVal() == z.getVal()) {
                    redundant = 1; return;
		}
		if (!x.isFixed() && !y.isFixed() && !z.isFixed()) return;
		core_key.push(prop_id);
		int sz = core_key.size();
		if (x.isFixed()) core_key.push(x.getVal());
		if (y.isFixed()) core_key.push(y.getVal());
		if (z.isFixed()) core_key.push(z.getVal());
		if (core_key.size() != sz+1) {
                    //x.print(); y.print(); z.print();
		}
		assert(core_key.size() == sz+1);
	}
...
public:
	...
	void getKey() {
		...
		if (y.isFixed() && z.isFixed()) {
			if (x.getMax() <= y.getVal()*z.getVal() && x.getMin() > y.getVal()*(z.getVal()-1)) {
				redundant = 1; return;
			}
		}
		if (!x.isFixed() && !y.isFixed() && !z.isFixed()) return;
