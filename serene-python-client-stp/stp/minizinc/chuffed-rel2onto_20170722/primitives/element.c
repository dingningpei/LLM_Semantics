...
public:
	...
	IntElemBounds(IntView<U> _y, IntView<V> _x, vec<IntView<W> >& _a) :
        ...
        no_min_support(false), no_max_support(false), has_fixed(0) {
	...
	int checkSatisfied() {
        ...
        //y = a[x]
        void getKey() { 
            if (x.isFixed() && y.isFixed()) return;
            if (!x.isFixed() && !y.isFixed() && !has_fixed) return;
            if (x.isFixed()) {
                core_key.push(prop_id);
                core_key.push(x.getVal());
            } else if (y.isFixed()) {
                core_key.push(prop_id);
                core_key.push(y.getVal());
            } else {
                core_key.push(prop_id);
                for (int i = 0; i < a.size(); i++) {
                    if (a[i].isFixed() && i >= x.getMin() && i <= x.getMax())
                        core_key.push(a[i].getVal());
                }
            }
        }
...
public:
	...
	IntElemDomain(IntView<U> _y, IntView<V> _x, vec<IntView<W> >& _a) :
        y(_y), x(_x), a(_a),key_sz((_a.size()+31)/32),fixed(new Tint[key_sz]), has_fixed(0) {
	...
	bool propagate() {
        ...
        void getKey() {assert(false); }
	/*void getKey() {
		if (y.isFixed() && x.isFixed()) return;
		if (!x.isFixed() && !y.isFixed() && !has_fixed) return;
		if (y.isFixed()) {
			core_key.push(prop_id | (0<<30));
			core_key.push(y.getVal());
		} else if (x.isFixed()) {
			core_key.push(prop_id | (1<<30));
			core_key.push(x.getVal());
		} else {
			core_key.push(prop_id | (2<<30));
			for (int i = 0; i < key_sz; i++) core_key.push(fixed[i]);
			for (int i = 0; i < sz; i++) {
				if (!inSet(i%32,fixed[i/32])) continue;
				core_key.push(a[i].getVal());
			}
		}
                }*/
