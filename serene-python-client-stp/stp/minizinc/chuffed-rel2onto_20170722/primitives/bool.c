...
//	NOT_SUPPORTED;
	...
	int u = 0;
        ...
        if (sat.project_no[var(x)] == -1) sat.project_no[var(x)] = sat.num_pvars++;
	if (sat.project_no[var(y)] == -1) sat.project_no[var(y)] = sat.num_pvars++;
	if (sat.project_no[var(z)] == -1) sat.project_no[var(z)] = sat.num_pvars++;
...
void bool_clause(vec<BoolView>& x, vec<BoolView>& y) {
	...
	for (int i = 0; i < y.size(); i++) ps.push(~y[i]);
        //printf("About to addClauseProj with a ps of lenght %d \n",ps.size());
        //sat.addClause(ps);
	sat.addClauseProj(ps);
}
