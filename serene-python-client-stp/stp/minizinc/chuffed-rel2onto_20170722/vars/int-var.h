...
class IntVar : public Var, public Keyable {
public:
	int const var_id;
	...
	bool in_scip;
	bool all_in_scip;
        //Tint decided; ////Just to test the late injection, tested on 37_coloring.fzn
        //int expl_appear;
...
// Engine stuff
	...
	int *key;     // where the real bit vector is
...
// Debug
	...
	void print();
		...
		b.choices.push(DecInfo(this, i, 1));
            }
        }
        ...
        void getSplitMinDec() {
            engine.branchingDescs.push();
            BranchingDesc& b = engine.branchingDescs.last();
            b.choices.push(DecInfo(this, (max+min)/2, 3));
            b.choices.push(DecInfo(this, (max+min)/2, 2));
        }
	...
	virtual int    getSize()    const { return max - min; }
	virtual int    getDegree()  const { return pinfo.size(); }
};
