...
IntVar::IntVar(int _min, int _max) :
	...
	, activity(0)
        , in_queue(false)
                    , in_key_queue(false)
                    ,unchanged(0)
                //,decided(0) //Just to test the late injection, tested on 37_coloring.fzn
{
...
void IntVar::initVals(bool optional) {
	...
	//* Original version by Geoffrey
        key_sz = (max>>5) - (min>>5) + 1;
	key = (int*) malloc(key_sz * sizeof(int));
	memset(key, 0xff, key_sz * sizeof(int));
	key[0] &= ~(bit32[min&31]-1);
	key[key_sz-1] &= bit32[(max&31)+1]-1;
	//vals = (Tchar*)key - (min>>5); //????
        //*/
        //key_sz = max-min+2;
...
void IntVar::pushInKeyQueue() {
	if (!in_key_queue) {
		in_key_queue = true;
		//engine.var_keys.push(this);
	}
}
...
void IntVar::getKey() {
	...
	in_key_queue = false;
	//if (!keyable) return;
	//if (!SHORT_DU && this == engine.opt_var) return;
	//if (!SHORT_DU && isFixed()) return;
	//if (unchanged) return;

        /* Original version by Geoffrey
        core_key.push(var_id);
	if (vals) {
		for (int i = 0; i < key_sz; i++) core_key.push(key[i]);
	} else {
		core_key.push(min);
		core_key.push(max);
	}
        ...
        core_key.push(var_id);
        int ckb = core_key.size();
        if (isFixed()) {
            //if (!decided) //Just to test the late injection, tested on 37_coloring.fzn
            //    core_key.push(getVal());
            return;

        }
        int is_set = 0;
        for (int i = getMin(); i <= getMax(); i++) if (!indomain(i)) is_set++;
	if (is_set) {
            //for (int i = 0; i < key_sz; i++) core_key.push(vals[i]);
            core_key.push(min0-is_set);
            for (int i = getMin(); i <= getMax(); i++) if (indomain(i)) core_key.push(i);
	} else {
		core_key.push(min);
		core_key.push(max);
	}
        assert(core_key.size() >= ckb + 2);


        /*
        if (vals) {
            dom_key.push(var_id | (3<<30));
            dom_key.push(key_sz);
            for (int i = 0; i < key_sz; i++) dom_key.push(vals[i]);
        } else {
            dom_key.push(var_id | (2<<30));
            dom_key.push(min);
            dom_key.push(max);
        }
        dom_key[0]++;
