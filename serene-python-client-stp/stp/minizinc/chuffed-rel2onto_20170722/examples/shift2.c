...
public:
  ...
  ShiftSched(int _staff, int _shifts, int _acts, vec< vec<int> >& _demand, int mode, MDDOpts& _mopts)
      : staff(_staff), shifts(_shifts), acts(_acts), dom(acts+maxG), demand(_demand), mopts(_mopts)
  {
...
//	assert(argc == 2 || argc == 3);
  ...
  engine.solve(new ShiftSched(staff,shifts,acts,demand,mode,mopts));
  return 0;
}
