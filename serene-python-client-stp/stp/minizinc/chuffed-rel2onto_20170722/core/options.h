...
public:
    ...
    int checkexpl; // 1-equalities, 2-inside can't reach out, 3-outside can't reach in, 4-smaller group
                    // can't reach bigger group, 5-bigger group can't reach smaller group
    int checkfailure; // 1-first, 2-smallest cycle, 3-largest cycle, 4-cycle with lowest ave level, 5-cycle with highest ave level, 6-cycle with lowest ave activity, 7-cycle with highest ave activity,
    ...
    int rootSelection; //(1-first non-fixed, 2-random unfixed, 3-end of shortest chain, 4-end of longest chain,
    // 5- start of shortest chain (+ collapse), 6-start of longest chain (+collapse))
    // 7- first (even if fixed), 8 - random (even if fixed), 9-largest domain, 10-all not self cycles
	...
	bool alldiff_stage;              // if bounds or domain consistency, put value propagator too
        ...
        bool check_prop; //Checks if propagators are satisfied

        //For TSPHeldKarp testing:
        bool give_naive_explanations_HK;
        bool use_last_conflict_HK;
        int wakeup_threshold_HK;
        bool use_circuit_kf_prop;
        ...
        int presolve;
        std::string mddfolder;
        bool presolve_test;
        std::string mdd_name;

        int mdd_prop_type;

        bool show_partial_solution;
        bool allow_overtime; //Allow going over the time limit if no solution has been found until the first is 
        bool soft_assumptions;

        int relax_every;
        int relax_every_mode;
	Options();
