...
public:
    ...
    //static std::unordered_map<Clause*, int> usage_count;
    //static std::unordered_map<Clause*, Clause*> gen_to_learnt;
    //static std::unordered_map<int, int> prop2contrib;//How many times a clause genrated by prop went into a learnt clause
    //static std::unordered_map<Clause*, int> clause2prop; //for each clause, who created it

	unsigned int learnt    : 1;             // is it a learnt clause
	unsigned int temp_expl : 1;             // is it a temporary explanation clause
        unsigned int mark      : 1;             //for caching
	unsigned int padding   : 5;             // save some bits for other bitflags
  ...
  Lit data[0];                            // the literals of the clause
	...
	float&       activity    ()              { return data2[sz]; }
        int&         project_no  ()              {return data[sz].x;}//{ return *((int*) &data[size()]); }
};
...
Clause* Clause_new(const V& ps, bool learnt = false) {
    assert(ps.size() > 0);
    //printf("Creating clause of size %d\n",ps.size());
    //ps.size()+1 because of Caching, it adds the proj_no at the end (based on original code)
    int mem_size = sizeof(Clause) + (ps.size()+1) * sizeof(Lit) + (learnt ? 1 : 0) * sizeof(int); 
...
struct LitFlags {
    ...
    unsigned int opt_var_lit : 1;
	unsigned int padding   : 4;            // leave some space for other flags    

    LitFlags(char f) { *((char*) this) = f; opt_var_lit = 0;}
    ...
    void setOptVarLit() { opt_var_lit = 1;}
};
...
public:
	...
	WatchElem() : a(0) {}
 WatchElem(Clause *c) : pt(c) { d.type = 0; if (sizeof(Clause *) == 4) d.d2 = 0; }
