...
template <typename T>
bool UF<T>::connected(int x, int y) const {
...
//explanations come from
template <typename T>
int RerootedUnionFind<T>::find(int val) const{
    assert(val >= 0);
    assert(val < size);
    int i = val;
    while (parents[i] != i) {
...
template <typename T>
bool RerootedUnionFind<T>::connected(int u, int v) const {
