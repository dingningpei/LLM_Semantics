...
Tint num_fixed;
	...
	AllDiffValue(vec<IntView<U> > _x, int _range) : 
sz(_x.size()), x(_x.release()), range(_range), num_fixed(0)
	{
		priority = 2; 
	...
	void wakeup(int i, int c) {		
            //x[34].var->log_calls_ = true;
            /*if (!x[i].isFixed()) {
                printf("IntVar %d [%d,%d]not fixed!\n",i, x[i].getMin(), x[i].getMax());
                printf("%d %d %d %d\n",x[i].var->changes,EVENT_LU, EVENT_F, EVENT_C);
                }*/
            assert(x[i].isFixed());
            new_fixed.push(i);
            pushInQueue();
...
Tint num_fixed;
 ...
 AllDiffBounds(vec<IntView<U> > _x, int _range) : sz(_x.size()), x(_x.release()), range(_range),num_fixed(0) {
		priority = 4;
		iv = new interval[sz];
...
Tint num_fixed;
 ...
 AllDiffDomain(vec<IntView<U> > _x, int _range) : sz(_x.size()), x(_x.release()), range(_range),num_fixed(0)
	{
