...
public:
    ...
    {      
        ...
        /*        for (int j = 0; j < 52; j++) {
            cout<<"   "<<j<<" ";
            for (IntView<>::iterator i = x[j].begin(); i != x[j].end(); ++i) {
                cout<<*i<<" ";
            }
            cout<<endl;
            }*/
    ...
    bool propagateSCC(int root) {
        ...
        for (IntView<>::iterator i = x[root].begin(); i != x[root].end(); ++i) 
        {
            int child = *i;
            //cout<<"Loops, child "<<child<<endl;
                ...
                for(int i = 0; i < notseen.size(); i++)
                {
                    ...
                    if(x[outsideVar].setValNotR(outsideVar))
                        if(!x[outsideVar].setVal(outsideVar, r)){
                            //cout<<"FALSE here3 "<<outsideVar<<endl;
                            return false;
                        }
                }
            }
...
void subcircuit_kf(vec<IntVar*>& _x) {
