...
void printLit(Lit p) {
    /*    if (TREEPROP_DEBUG) { 
        if (sat.isRootLevel(var(p))){
            //printf("RootLevel");
            //return;
        }
        printf("%d:", tr[var(p)]);
        ChannelInfo& ci = sat.c_info[var(p)];
        if (ci.cons_type == 1) {
            ...
            //printf(":%d:%d, ", sign(p), sat.trailpos[var(p)]);
        }
        }*/
}
void printClause(Clause* _c) { 
    if (TREEPROP_DEBUG) {
        if(_c == NULL) return;
        Clause& c = *_c;
...
int TreePropagator::articulations(int n, bool reachable[], int& count) {
    ...
    _findAndBuildBridges(n,count,s,depth,low,reachable,parent,hits,hits_count,
    ...
    bool toBePropagated[nbNodes()];
    std::memset(toBePropagated, false, sizeof(bool)*nbNodes());
    if (bridgeExpl.size() > 0) {
        ...
        for (it = bridgeExpl.begin(); it != bridgeExpl.end(); it++){
            ...
            if (!isArt && !getEdgeVar(bridge).setValNotR(true)) {
                toBePropagated[getEndnode(bridge,0)] = true;
                toBePropagated[getEndnode(bridge,1)] = true;
                nodeForcedBridge.push_back(std::make_pair(getEndnode(bridge,0),bridge));
                nodeForcedBridge.push_back(std::make_pair(getEndnode(bridge,1),bridge));
            ...
            if (isArt && toBePropagated[bridge]) 
                continue;
    ...
    return hits_count;//nbBridgesBuilt;
...
void TreePropagator::precycle_detect(int unk_edge) {
    ...
    if (uf.connected(u,v)) {
        ...
        getEdgeVar(unk_edge).setVal(false,r);
...
void TreePropagator::_findAndBuildBridges(int u, int& count, std::stack<edge_id>& s,
    ...
    for (int i = 0; i < adj[u].size(); i++) {
        ...
        if (!visited[v]){
            ...
            _findAndBuildBridges(v,count,s,depth,low,visited,parent,hits,hits_count,
            ...
            if (low[v] >= depth[u]) { //u is articulation
                ...
                if (prevHitsTop != hits.top()) {
                    ...
                    if(cnt == 1 && !getEdgeVar(e2).isFixed()) {
                        //For parallel edges: they would not appear in the do{}while above
                        int u = getEndnode(e2,0);
                        int v = getEndnode(e2,1);
                        bool parallel = false;
                        for (int a = 0; a < adj[u].size(); a++) {
                            if (adj[u][a] != e2 && (!getEdgeVar(adj[u][a]).isFixed() || !getEdgeVar(adj[u][a]).isFalse())) {
                                parallel = true;
                                break;
                            }
                        }
...
TreePropagator::TreePropagator(vec<BoolView>& _vs, vec<BoolView>& _es, 
    ...
    : GraphPropagator(_vs,_es,_en),uf(_vs.size()), ruf(_vs.size()),nb_innodes(0) {

    tree_propagators.push_back(this);

    was_counted = new Tint[nbNodes()];
...
void TreePropagator::wakeup(int i, int c) {
    ...
    if(i >= nbNodes() && i < nbNodes()+nbEdges()) { 
        ...
        //EVENT ON NODES
        if (getNodeVar(i).isTrue() && was_counted[i] == 0) {
            nb_innodes++;
            was_counted[i] = 1;
        }
...
bool TreePropagator::propagate() {
    ...
    for (uint i = 0; i < nodeForcedBridge.size(); i++) {
        int n = nodeForcedBridge[i].first;
        int e = nodeForcedBridge[i].second;
        assert(getEdgeVar(e).isFixed() && getEdgeVar(e).isTrue());
        if (!getNodeVar(n).isFixed()) {
            Clause* r =  NULL;
            if (so.lazy) {
                vec<Lit> ps; ps.push();
...
bool TreePropagator::propagateNewNode(int node) {
    ...
    if (newNodeCompleteCheckup) {
        hits_count = articulations(node,blue,count);
        didBlueDFS = true;
...
bool TreePropagator::propagateRemEdge(int edge) {
        ...
        if (degree == 0){ //The node is left all alone
            ...
            if(getNodeVar(node).isFixed() && getNodeVar(node).isTrue()) {
                ...
                if (nb_innodes >= 1) {
                    ...
                    getNodeVar(node).setVal(false,r);                        
    ...
    for (int i = 0 ; i < 2; i++){
        ...
        std::fill(visited, visited + sizeof(visited), false);
...
void branch(vec<Branching*> x, VarBranch var_branch, ValBranch val_branch) {
    if (var_branch == PRIM_ORDER) {
        engine.branching->add(new PrimStrategy(x, var_branch, true));
        return;
    }
