...
// Dummy propagator for projection
class Table : public Propagator {
public:
	int const sz;
	IntView<> * const x;
	...
	Table(vec<IntView<> > _x)	: sz(_x.size()), x(_x.release()), num_fixed(0) {
            for (int i = 0; i < sz; i++) x[i].attach(this,i, EVENT_F);
	}

	void wakeup(int i, int c) {
		trailChange(num_fixed, num_fixed+1);
	}
	...
	void getKey() {
		...
		for (int i = 0; i < sz; i++) {
			core_key.push(x[i].isFixed() ? x[i].getVal() : INT_MAX);
		}
	}
...
void table(vec<IntVar*>& x, vec<vec<int> >& t) {
	vec<IntView<> > _x;
	for (int i = 0; i < x.size(); i++) _x.push(IntView<>(x[i]));
	new Table(_x);
	table_GAC(x, t);
}
