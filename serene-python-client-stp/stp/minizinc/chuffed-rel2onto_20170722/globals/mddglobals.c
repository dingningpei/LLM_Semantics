...
static MDDProp<4>* addMDDProp(vec<IntView<4> >& x, MDDTable& tab, _MDD m, const MDDOpts& mopts);
...
void addMDD(vec<IntVar*>& x, MDD m, const MDDOpts& mdd_opts)
{
  ...
  if(m.val == m.table->fff().val)
    ERROR("Added False MDD.");

//  m.table->print_mdd(m.val);

  if(mdd_opts.decomp == MDDOpts::D_PROP)
  {
    addMDDProp(x, *(m.table), m.val, mdd_opts);
  } else { // ENCODINGS used from mdd_opts
    mdd_decomp_dc(x, *(m.table), m.val, mdd_opts);
  }

}

MDDProp<4>* addMDD(vec<IntView<4> >& x, MDD m, const MDDOpts& mdd_opts)
{
  if(m.val == m.table->ttt().val)
    return NULL;
...
//  m.table->print_mdd(m.val);
  ...
  if(mdd_opts.decomp == MDDOpts::D_PROP)
  {
    return addMDDProp(x, *(m.table), m.val, mdd_opts);
  } else { // ENCODINGS used from mdd_opts
      mdd_decomp_dc(x, *(m.table), m.val, mdd_opts);
  }
  return NULL;
...
static MDDProp<4>* addMDDProp(vec<IntView<4> >& x, MDDTable& tab, _MDD m, const MDDOpts& mopts)
{
   ...
   for (int i = 0; i < x.size(); i++) x[i].var->specialiseToEL();

   MDDTemplate* templ = new MDDTemplate(tab, m, doms);

   return new MDDProp<4>(templ, x, mopts);
}
