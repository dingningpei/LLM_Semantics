...
class IntegrationGraph(object):
    ...
    def __init__(self, octopus, dataset, match_threshold=0.0,
        ...
        self._chuffed_path, self._mzn2fzn_exec, self._model_mzn, self._solns2out_exec, self._timeout = \
            self._read_config()
        print("self._chuffed_path111: ", self._chuffed_path)
    ...
    def _get_initial_alignment(self, alignment_graph,
        ...
        # print("num_attrs: ", graph.nodes(data=True))
        unknown_dn = [n_id for (n_id, n_data) in graph.nodes(data=True) if n_data["attr_dict"]["type"] == "DataNode" and
                      n_data["attr_dict"]["label"] == UNKNOWN_CN + "---" + UNKNOWN_DN]

        unknown_cn = [n_id for (n_id, n_data) in graph.nodes(data=True) if n_data["attr_dict"]["type"] == "ClassNode" and
                      n_data["attr_dict"]["label"] == UNKNOWN_CN]
        if len(unknown_cn) > 1:
        ...
        if len(unknown_cn) < 1:
            if add_unknown:
                ...
                class_nodes = [n_id for (n_id, n_data) in graph.nodes(data=True) if n_data["attr_dict"]["type"] == "ClassNode"]
                ...
                for (n_id, inc) in enumerate(class_nodes):
                    link_data["alignId"] = DEFAULT_NS + ALL_CN + "1---" + \
                                           DEFAULT_NS + OBJ_PROP + "---" + \
                                           DEFAULT_NS + graph.nodes[n_id]["attr_dict"]["label"]
    ...
    def _add_matches(self, column_names=None):
        ...
        for n_id, n_data in self.graph.nodes(data=True):
            # print("n_id, n_data: ", n_id, n_data)
            if n_data["attr_dict"]["type"] == "DataNode":
                data_node_map[n_data["attr_dict"]["label"]].append(n_id)
        ...
        if self._add_unknown and "scores_" + UNKNOWN_CN + "---" + UNKNOWN_DN not in predicted_df.columns:
            ...
            predicted_df.loc["scores_" + UNKNOWN_CN + "---" + UNKNOWN_DN] = 0.0
    ...
    def _alignment2dzn(self):
        ...
        # print("self.graph: ", self.graph.edges(data=True))
        def sanitize_graph_flatten_dict(graph):
            sanitized_graph = nx.MultiDiGraph()
            
            # Sanitize node attributes
            # for node, data in sanitized_graph.nodes(data=True):
            #     keys_to_add = {}
            #     keys_to_remove = []
            ...
            #                 keys_to_add[new_key] = sub_value
            #             keys_to_remove.append(key)
            #     for key in keys_to_remove:
            #         del sanitized_graph.nodes[node][key]
            #     sanitized_graph.nodes[node].update(keys_to_add)
            
            # Sanitize edge attributes
            for node, data in graph.nodes(data=True):
                sanitized_graph.add_node(node, type=data["attr_dict"]["type"], label=data["attr_dict"]["label"])
            for u, v,k, data in graph.edges(data=True, keys=True):
                sanitized_graph.add_edge(u, v, keys=k, weight=data["attr_dict"]["weight"])
            
            return sanitized_graph

        sanitized_G = sanitize_graph_flatten_dict(self.graph)

        nx.write_graphml(sanitized_G, alignment_path)
    ...
    def _matches2dzn(self, column_names=None):
        ...
        nx.write_graphml(sanitize_graph_flatten_dict(integration_graph), integration_path)
    ...
    def _generate_oznfzn(self, column_names=None):
        ...
        print("before command: ",self._chuffed_path)
        command = "{} -c --solver org.minizinc.mzn-fzn -I {}/globals/ {} {} {} --fzn {}.fzn --ozn {}.ozn".format(self._mzn2fzn_exec, self._chuffed_path, self._model_mzn,
        ...
        def find_weight(links):
            ...
            if len(links) == 1:
                for key, val in links.items():
                    # print("key: ", key)
                    # print("val: ", val)
                    # print("links: ", links[key])
                    # print("inks[key]: ", links[key]["attr_dict"]["type"])
                    return links[key]["attr_dict"]
            elif len(links) > 1:
                # we need to find the link with weight
                # we take the first match!
                for key, val in links.items():
                    if abs(val["attr_dict"]["weight"] - weight) < 0.001:
                        print("val: ", val)
                        return val["attr_dict"]
            else:
        ...
        for ind, ed in enumerate(ag.edges_iter()):
            ...
            if start not in attributes and end not in attributes:
                ...
                if start in attributes:
                    column_id = copy_graph.nodes[start]["attr_dict"]["columnId"]
                    ...
                    # print(copy_graph.nodes[end])
                    column_id = copy_graph.nodes[end]["attr_dict"]["columnId"]
    ...
    def solve(self, column_names=None, dot_file=None,
        ...
        logging.info("Solving the problem for dataset {} using octopus {}".format(
            self._dataset.id, self._octopus.id))
        # if soft_assumptions:
        #     command = "{} --verbosity 2 -t {} --lazy on " \
        #               "{}.fzn | {} -o {}.ozn".format(self._chuffed_path,
        #                                                           self._timeout,
        #                                                           base_path,
        ...
        if soft_assumptions:
            command = "{}/fzn_chuffed -verbosity=2 -time_out={} -lazy=true " \
                      ...
                      "-steinerlp=true {}.fzn | {} --ozn-file {}.ozn".format(self._chuffed_path,
                                                                  self._timeout,
                                                                  relax_every,
                                                                  base_path,
                                                                  self._solns2out_exec,
